...
short={"APR":f"APR\n(UAE Arab)\nn={n['APR']}","JaSaPaGe-Saudi":f"JaSaPaGe\nSaudi\nn={n['JaSaPaGe-Saudi']}","JaSaPaGe-Japanese":f"JaSaPaGe\nJapanese\nn={n['JaSaPaGe-Japanese']}","HPRC_r2":f"HPRC r2\n(mixed)\nn={n['HPRC_r2']}"}
one={"APR":"APR","JaSaPaGe-Saudi":"Saudi","JaSaPaGe-Japanese":"Japanese","HPRC_r2":"HPRC r2"}
fig=plt.figure(figsize=(16,11.5)); gs=fig.add_gridspec(2,3,height_ratios=[3,1.3],hspace=0.5,wspace=0.35)
...
for ax,gene in zip(axes[:3],["HLA-A","HLA-B","HLA-DRB1"]):
    ...
    ax.set_yticks(range(len(f))); ax.set_yticklabels(f.index,fontsize=9); ax.set_xticks(range(4)); ax.set_xticklabels([short[c] for c in order],fontsize=8); ax.set_title(f"{gene} two-field alleles",fontsize=11)
    for i in range(len(f)):
        for j in range(4):
            v=f.values[i,j]
            if v>0: ax.text(j,i,f"{v:.2f}",ha="center",va="center",fontsize=7,color="white" if v>0.2 else "black")
...
ax.set_xticklabels([one[c] for c in order],rotation=0,fontsize=9); ax.set_ylabel("fraction of haplotypes"); ax.set_title("Secondary DRB gene (DR haplogroup)",fontsize=10); ax.legend(fontsize=7,ncol=2,loc="lower left"); ax.set_xlabel("")
ax=axes[4]; c4=cn.groupby("cohort")[["C4AL","C4AS","C4BL","C4BS"]].mean().reindex(order)
c4.plot.bar(ax=ax,width=0.8,color=["#d62728","#ff9896","#9467bd","#c5b0d5"]); ax.set_xticklabels([one[c] for c in order],rotation=0,fontsize=9); ax.set_ylabel("mean copies per haplotype"); ax.set_title("C4A / C4B, long (L) and short (S) forms",fontsize=10); ax.legend(fontsize=7,ncol=2); ax.set_xlabel("")
...
nv.plot.bar(ax=ax,width=0.8,color=["#1f77b4","#d62728","#2ca02c","#7f7f7f"]); ax.set_xticklabels([g.replace("HLA-","") for g in cl],rotation=0,fontsize=8); ax.set_ylabel("fraction of gene copies"); ax.set_title("Full-length allele absent from IPD-IMGT/HLA 3.55",fontsize=10); ax.legend([one[c] for c in order],fontsize=7,ncol=2); ax.set_xlabel("")
fig.suptitle(f"HLA allele landscape across {sum(n.values())} assembled haplotypes (Immuannot on assembly-derived MHC)",fontsize=13,y=0.98)
